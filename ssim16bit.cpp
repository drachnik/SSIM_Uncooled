...
// Function to read 16-bit image using stb_image
std::vector<double> read_image_16bit(const std::string& filename, int& width, int& height, int& channels) {
    unsigned short* data = stbi_load_16(filename.c_str(), &width, &height, &channels, 1); // Force to load as grayscale
    if (!data) {
...
double ssim(const std::vector<double>& img1, const std::vector<double>& img2) {
    ...
    const double L = 65535.0; // Since pixel values are in the range [0, 65535]
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::vector<double> img1 = read_image_16bit(argv[1], width1, height1, channels1);
        std::vector<double> img2 = read_image_16bit(argv[2], width2, height2, channels2);
